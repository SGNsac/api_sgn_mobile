...
const selectUser = (usuaSigla: any) => {
  return "SELECT USUA_SIGLA FROM USUARIO WHERE USUA_SIGLA = '" + usuaSigla + "'"
}

const login = (login: string) => {
  return "SELECT USUA_SIGLA,USUA_SENHA_APP FROM USUARIO WHERE USUA_SIGLA = '" + login + "'"
}

const update = (user: string, passwordHash: string) => {
  return "UPDATE USUARIO SET USUA_SENHA_APP = '" + passwordHash + "' WHERE USUA_SIGLA = '" + user + "'"
}
